...
class Parser
{
  ...
  protected function parseMethods(SimpleXMLElement $class) {
    $methods = array();
    $className = (string)$class->full_name;
    $className = preg_replace('/^[^\w]+/i', '', $className);
    foreach ($class->method as $method) {
      $methodName = (string)array_pop(explode('::', $method->full_name));
      $return = $method->xpath('docblock/tag[@name="return"]');
      $return = count($return) ? (string)str_replace(array('|\\', '|\\\\'), '|', preg_replace('/^[^\w]+/i', '', $return[0]['type'])) : 'mixed';
      $arguments = array();
      foreach ($method->argument as $argument) {
        $types = implode('', array_map(function($a){
          return preg_replace('/^[^\w]+/i', '', $a);
        }, explode('|', $argument->type)));
        $nArgument = array(
          'type' => (string)$types,
      ...
      $argumentStr = implode(', ', array_map(function ($argument) {
        return ($argument['type'] ? sprintf('%s ', str_replace(array('|\\', '|\\\\'), '|', $argument['type'])) : '') . $argument['name'];
      }, $arguments));
      $signature = sprintf('%s %s::%s (%s)', $return, $className, substr($methodName, 0, -2), $argumentStr);
      $methods[$methodName] = array(
        'name' => $methodName,
        'description' => trim(str_replace("\n", ". ", implode("\n", array(trim((string)$method->docblock->description), trim((string)$method->docblock->{"long-description"}))))),
  ...
  protected function parseProperties(SimpleXMLElement $class) {
    ...
    foreach ($class->property as $xProperty) {
      ...
      if (count($xVar)) {
        $type = str_replace(array('|\\', '|\\\\'), '|', preg_replace('/^[^\w]+/i', '', $xVar[0]->type));
      }
      ...
      $properties[$propName] = array(
        ...
        'type' => $type,
        'default' => $default,
        'description' => trim(str_replace("\n", ". ", implode("\n", array(trim((string)$xProperty->docblock->description), trim((string)$xProperty->docblock->{"long-description"}))))),
  ...
  protected function parseConstants(SimpleXMLElement $class) {
    ...
    foreach ($class->constant as $xConstant) {
      ...
      $constants[$name] = array(
        'name' => $name,
        'description' => preg_replace('/\n+/i', '. ', array((string)$xConstant->docblock->description . ". " . (string)$xConstant->docblock->{"long-description"})),
        'signature' => $signature,
        'value' => $value,
        'deprecated' => count($class->xpath('docblock/tag[@name="deprecated"]')) > 0,
        'definedBy' => $className,
